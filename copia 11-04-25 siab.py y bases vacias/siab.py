...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS actividades (
        ...
        firma_bombero_usuario TEXT,
        firma_bombero_fecha TEXT,

        firma_supervisor_usuario TEXT,
        firma_supervisor_fecha TEXT,
    ...
    # -------------------------------------------------
    # MIGRACIONES SEGURAS (SOLO SI FALTAN COLUMNAS)
    # -------------------------------------------------

    def agregar_columna(tabla, columna_def):
        try:
    ...
    # Usuarios
    agregar_columna("usuarios", "legajo TEXT")
    agregar_columna("usuarios", "activo INTEGER DEFAULT 1")
    agregar_columna("usuarios", "fecha_baja TEXT")
    agregar_columna("usuarios", "usuario_baja TEXT")
    agregar_columna("usuarios", "debe_cambiar_password INTEGER DEFAULT 1")

    # Actividades (por si la DB ya existía vieja)
    agregar_columna("actividades", "concepto_id INTEGER")
    agregar_columna("actividades", "horas REAL")
    agregar_columna("actividades", "fecha_creacion TEXT")
    agregar_columna("actividades", "fecha_modificacion TEXT")
    agregar_columna("actividades", "creado_por INTEGER")
    agregar_columna("actividades", "modificado_por INTEGER")

    agregar_columna("actividades", "firma_bombero_usuario TEXT")
    agregar_columna("actividades", "firma_bombero_fecha TEXT")
    agregar_columna("actividades", "firma_supervisor_usuario TEXT")
    agregar_columna("actividades", "firma_supervisor_fecha TEXT")

    agregar_columna("actividades", "anulada INTEGER DEFAULT 0")
    agregar_columna("actividades", "usuario_anula TEXT")
    agregar_columna("actividades", "fecha_anulacion TEXT")
    agregar_columna("actividades", "motivo_anulacion TEXT")
...
class LoginWindow:
    def __init__(self, master):
        ...
        # 🔴 OCULTAR antes de configurar
        self.top.withdraw()

        # 🔴 aplicar icono inmediatamente
        if hasattr(master, "_icono_global") and master._icono_global:
            self.top.iconphoto(True, master._icono_global)

        # resto normal
        self.top.title("Login SIAB")

        self.top.configure(bg="#f0f0f0") # Sugerencia: color de fondo
                
        # 3. Llamar al icono y mostrar la ventana con un delay mínimo
        # 100 milisegundos son suficientes para que Windows reaccione
        self.top.after(100, self._preparar_ventana)
        
        Label(self.top, text="Usuario:").grid(row=0, column=0, padx=10, pady=10)
        Label(self.top, text="Contraseña:").grid(row=1, column=0, padx=10, pady=10)

        self.user = Entry(self.top)
        self.pwd = Entry(self.top, show="*")
        self.user.grid(row=0, column=1, padx=10, pady=10)
        self.pwd.grid(row=1, column=1, padx=10, pady=10)

        self.btn_login = Button(self.top, text="Entrar", command=self.login)
        ...
        self.top.after(150, _forzar_foco)
    ...
    def login(self):
        ...
        c.execute("""
            ...
            WHERE LOWER(u.username)=? AND u.activo=1
        """, (user,))
    ...
    def abrir_ventana_cambio_password(self, user):

        win = tk.Toplevel(self.top)
        win.title("Cambio Obligatorio de contraseña")

        # 🔒 hacerlo modal y encima del login
        win.transient(self.top)
        win.grab_set()
        win.focus_force()

        # 📍 centrar después de esto
        self.centrar_ventana(win, 400, 220)
        win.resizable(False, False)

        # Bloquear cierre con X
        win.protocol("WM_DELETE_WINDOW", lambda: None)

        Label(win, text="Debe cambiar su contraseña",
            font=("Arial", 12, "bold")).pack(pady=15)

        Label(win, text="Nueva contraseña:").pack()
        var_new = StringVar()
        e_new = Entry(win, textvariable=var_new, show="*", width=30)
        e_new.pack(pady=5)

        Label(win, text="Confirmar contraseña:").pack()
        var_conf = StringVar()
        e_conf = Entry(win, textvariable=var_conf, show="*", width=30)
        e_conf.pack(pady=5)
        ...
        # 🔹 ENTER pasa del primer campo al segundo
        e_new.bind("<Return>", lambda e: e_conf.focus_set())

        # 🔹 ENTER en confirmar ejecuta guardar
        e_conf.bind("<Return>", guardar)

        # 🔹 ENTER en toda la ventana ejecuta guardar
        e_new.focus_set()
...
class App:
    def __init__(self, master, usuario_actual):
        ...
        self.icono_path = os.path.join(base_dir, "Bomberos.png")
    ...
    def _aplicar_permisos(self):
        if getattr(self, "cerrando", False):
            return  # Si estamos cerrando, no hacer nada
        ...
        if hasattr(self, "leg_btns"):
            ...
            if rol == "BOMBERO":
                ...
                # ADMIN: no tocar botones acá
                # el estado (inicial / cargado / nuevo) los controla
                pass

         # === CONCEPTOS ===
        if rol == "BOMBERO" and hasattr(self, "conc_btns"):
            # Deshabilitar todos los botones de conceptos
            ...
            # Recorremos por clave y por texto visible del botón
            for clave, btn in self.conc_btns.items():
                texto = clave.lower()
                visible = btn.cget("text").lower() if hasattr(btn, "cget") else ""
                if (
                    "buscar" in texto
                    or "imprimir" in texto
                    or "buscar" in visible
                    or "imprimir" in visible
                ):
                    btn.config(state="normal")

        elif rol == "SUPERVISOR":
            # Puede todo menos eliminar
        ...
        # --- Refuerzo: asegurar que Supervisor pueda usar 'Nuevo' en Conceptos ---
        if rol == "SUPERVISOR" and hasattr(self, "conc_btns"):
            if "Nuevo" in self.conc_btns:
                self.conc_btns["Nuevo"].config(state="normal")
        ...
        if rol != "ADMIN":
            try:
                ...
                try:
                    if self.tab_control.winfo_exists():
                        idx = self.tab_control.index(self.usuarios_frame)
        ...
        # === Refuerzo adicional: bloquear campos si Bombero ===
        if rol == "BOMBERO":
            # Bloquear campos de legajo
            for w in [
            ...
            # Asegurar que fecha de baja esté deshabilitada
            try:
        ...
        try:
            {
                k: v.cget("state")
                for k, v in self.leg_btns.items()
                if v.winfo_exists()
            }
        except Exception:
            pass
...
#        from tkinter.messagebox import showerror
    ...
    def cargar_legajos_combobox(self):
            ...
            # 1. Obtenemos todos los legajos activos de la DB (solo las columnas que existen)
            conn = sqlite3.connect(DB_PATH)
            conn.row_factory = sqlite3.Row 
            c = conn.cursor()
            c.execute("""
            ...
            for r in rows:
                ...
                # 2. Buscamos el rol en el diccionario de usuarios que ya tenés cargado
                # Si no está en el dict, por seguridad lo tratamos como Bombero o lo ignoramos
                user_data = self.usuarios_dict.get(legajo_id, {})
                rol_iterado = user_data.get("rol", "").upper()

                # --- APLICAR FILTRO DE JERARQUÍA ---
                if rol_usuario == "SUPERVISOR":
                    # El Supervisor NO ve a los ADMIN ni a OTROS SUPERVISORES (excepto a él mismo)
                    if rol_iterado in ("ADMIN", "SUPERVISOR") and legajo_id != str(self.usuario_actual.get("legajo")):
                        continue
            ...
            if rol_usuario == "BOMBERO":
                ...
                valor_propio = f"{legajo} - {apellido} {nombre}".upper()
    ...
    def cargar_grilla_legajos(self):
        ...
        if rol == "BOMBERO":
            ...
            if not legajo_propio:
                conn.close()
                return

            c.execute("""
                SELECT legajo, apellido, nombre, grado, cargo, situacion
                FROM legajos
                WHERE legajo = ?
            """, (legajo_propio,))
...
def set_icon(window):
    ...
    try:
        ...
        icono = os.path.join(base_dir, "Bomberos.png")

        if os.path.exists(icono):
            icon_img = tk.PhotoImage(file=icono)
            window._icon_img = icon_img  # 🔥 clave (evita garbage collector)
...
if __name__ == "__main__":
    ...
    # 6. App principal
    app = App(root, login.logged_user)
